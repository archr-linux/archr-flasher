...
impl<T: ReadWriteSeek> FileSystem<T> {
    ...
    pub fn new(mut disk: T, options: FsOptions) -> io::Result<Self> {
        ...
        // ArchR: trust the BPB layout (sectors_per_fat_16 == 0 means FAT32)
        // instead of the cluster-count heuristic; see boot_sector.rs.
        let fat_type = if bpb.is_fat32() { FatType::Fat32 } else { FatType::from_clusters(total_clusters) };
