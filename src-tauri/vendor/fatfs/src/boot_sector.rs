...
impl BiosParameterBlock {
    ...
    fn validate(&self) -> io::Result<()> {
        ...
        let fat_type = if is_fat32 { FatType::Fat32 } else { FatType::from_clusters(total_clusters) };
        // ArchR: the R36S boot partition is FAT32 formatted with a cluster
        // count below the spec minimum (vendor U-Boots require exactly this
        // geometry), so the strict cross-check against from_clusters is
        // relaxed and the BPB layout signal wins.
